...
rf_rmse <- sqrt(mean((test$position_numeric - pred_rf)^2))
...
- Analyze comprehensive Formula 1 data (1950–2024)
...
```{r coefficients}
coefs_lm <- coef(lm_model)
coefs_lasso <- as.numeric(coef(lasso_model))

coef_compare <- data.frame(
  Feature = names(coefs_lm),
  `Linear Regression` = round(coefs_lm, 6),
  `Lasso` = round(coefs_lasso, 6),
  check.names = FALSE,
  row.names = NULL
)
...
## Prediction Accuracy: Linear vs Lasso

```{r predictions_plot_lm_lasso, fig.width=12, fig.height=5}
...
## Prediction Accuracy: Gradient Boosting vs Random Forest

```{r predictions_plot_gbm_rf, fig.width=12, fig.height=5}
compare_df <- data.frame(
  actual  = test$position_numeric,
  pred_gbm = pred_gbm,
  pred_rf  = pred_rf
...
4. **Model Choice** - Gradient Boosting captures non-linear relationships better than linear models, providing the best accuracy.
...
- Limited driver skill metrics beyond wins
- No tire compound or detailed strategy data

**Future Improvements:**

- Incorporate weather and track characteristics
- Add more granular driver performance metrics
- Include detailed pit stop strategy and tire data
...
✓ Machine learning can predict F1 races with high accuracy
